...
const EnhancedTokenTransfer = ({ showToast, showProgress, updateProgress, hideProgress }) => {
  ...
  // Check if balance is sufficient and show swap option
  useEffect(() => {
  ...
  const handleSwapAndTransfer = async () => {
    ...
    try {
      ...
      const swapTx = await swapETHForToken(
        ...
        (amountWei * BigInt(95) / BigInt(100)).toString(), // 5% slippage
        wallet.address
      );
  ...
  const handleDirectTransfer = async () => {
    ...
    try {
      ...
      // Check if approval is needed (for non-native transfers)
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if we need to swap or can transfer directly
    const required = parseFloat(form.amount);
  ...
  return (
    <div className="bg-gradient-to-br from-purple-50 to-blue-50 border border-purple-200 rounded-xl p-6">
      <div className="flex items-center mb-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-purple-900">增强版代币转账系统</h3>
...
export default EnhancedTokenTransfer;