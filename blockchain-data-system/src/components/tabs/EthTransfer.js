...
const EthTransfer = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (isSelfTransfer()) {
      showToast(
        '⚠️ 注意：向自己转账时某些网络可能不支持附加数据。如果失败，请尝试转账到其他地址。',
        'warning'
      )
    }
    ...
    try {
      ...
      addRecord({
        ...
        extra: isSelfTransfer()
          ? '自转账'
          : `转账到 ${form.address.slice(0, 6)}...${form.address.slice(-4)}`
      })

      setTimeout(() => {
        hideProgress()
        const successMessage = txParams.data
          ? '✅ ETH转账 + 数据上链成功！'
          : '✅ ETH转账成功！'
