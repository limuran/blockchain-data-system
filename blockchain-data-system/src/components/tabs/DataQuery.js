...
🏷️ 数据类型: ${dataTypeParam}`
      ...
      return `🔢 十六进制数据 (${byteLength} 字节): ${data.slice(0, 42)}${
        data.length > 42 ? '...' : ''
      }`
    } catch (error) {
  ...
  const handleGraphQuery = async () => {
    ...
    try {
      ...
      // 临时简化版本测试:
      const simpleTransform = (graphResponse) => {
        if (graphResponse?.data?.dataEntries) {
          return graphResponse.data.dataEntries.map((entry, index) => ({
            hash: entry.transactionHash,
            from: entry.userAddress,
            to: '0xcfb9534B39a81271464dF275b2ADBD697cB3b293',
            value: '0', // 以wei为单位
            gasUsed: 'N/A',
            gasPrice: 'N/A',
      ...
      // 使用简化版本测试:
      setQueryResults(simpleTransform(result))
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, graphqlQuery: e.target.value }))
                }
